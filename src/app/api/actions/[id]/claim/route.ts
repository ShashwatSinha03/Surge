...
import { claimActionService } from '@/features/actions/services/claimAction';
...
export async function POST(
  ...
  if (!canTransition(action.status, 'claimed')) {
    return NextResponse.json(
      { error: `Cannot claim action from ${action.status}.` },
      { status: 422 }
    );
  }
  ...
  return NextResponse.json({ message: 'Action claimed.' });
}
